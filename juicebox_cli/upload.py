...
class S3Uploader:
    def __init__(self, files, endpoint=None, netrc=None):
        ...
        if not self.jb_auth.client_id:
            logger.error(
                "Could not determine client ID from token. Please ensure your token contains a 'client' claim."
            )
    ...
    def get_s3_upload_token(self):
        ...
        if (
            self.endpoint and "dev" in self.endpoint.lower()
        ):  # Check if 'dev' is in the endpoint URL
            determined_env = "dev"
        logger.debug(
            f"Determined environment: {determined_env} from endpoint: {self.endpoint}"
        )  # New debug log
        ...
        if response.status_code == 401:
            ...
            logger.debug(response)
            logger.error(
                f"Failed to get S3 upload token. Status: {response.status_code}, Response: {response.text}"
            )
    ...
    def upload(self, app=None):
        ...
        if not client_id:
            logger.error(
                "Client ID could not be found in STS upload token response 'included' section."
            )
        ...
        for upload_file in self.files:
            ...
            if os.path.isdir(upload_file):
                logger.debug("%s: is a directory, scanning recursively", upload_file)
